...
`[g4f 免费 provider 复测] 更新原因：BlackForestLabs_Flux1Dev/StabilityAI_SD35Large 持续报 GPU 配额耗尽，需要复测替代方案；同时全量搜寻新的免费文本 provider。调整内容：1. 两者共享的 HuggingFace ZeroGPU 免费配额池已被全局耗尽（连续多轮 100% 复现），复测未找到免 Key 替代品，已从 IMAGE_PROVIDERS/IMAGE_PROVIDER_MODELS_MAP 移除，图片 provider 剩 PollinationsImage/AnyProvider/OperaAria 三个。2. 全量扫描后新增 3 个免费文本 provider 到 G4F_PROVIDERS/PROVIDER_MODELS_MAP：CohereForAI_C4AI_Command（command-a-03-2025/command-r-08-2024）、Groq（openai/gpt-oss-120b）、OpenRouterFree（openrouter/free），均补了 ROUTE_PROMPTS_MAP 人设和 PEER_REVIEW_PROMPTS_MAP 裁判提示词。3. 排除的候选：Perplexity（~50% 概率触发 g4f 库自身的 JsonConversation 报错，不稳定）、OllamaSwarm（响应夹带未剥离的 `<think>` 推理原文，且经常 TTFT 超时）。测试见 tests/test_provider_registry_new_providers.py，availability_g4f/ 下的调研脚本与结论文档已同步更新。`

`[Frontier-only mode] 更新原因：允许用户一键锁死免费 g4f provider，只用前沿 provider（Claude/ChatGPT/Gemini 等）输出。调整内容：1. index.html 对话表单和图片表单各新增一个 `.btn-frontier-toggle` 按钮（`#frontierOnlyToggle`/`#frontierOnlyToggleImage`），点击后禁用并强制取消勾选所有免费 provider 复选框（`.is-locked` 灰化），登录态守卫与其它前沿卡片同构（游客/匿名整卡禁用）。2. 提交时若该模式开启且未勾选任何前沿 provider，前端 alert 拦截，不发请求。3. `/api/compare`、`/api/generate-images` 新增请求体字段 `frontier_only`（默认 false）：为 true 时 `providers_to_test` 强制为空，跳过整个 g4f 并发阶段（不再触发历史遗留的"providers 数组为空=测试全部"语义），但仍正常调用 `save_chat_history()`/`save_image_history()` 写入空结果的历史记录，供后续前沿调用追加。测试见 tests/test_frontier_only_mode.py。`

`[前端视觉打磨] 更新原因：Frontier-only mode 锁定范围要覆盖免费模型下拉框而不只是勾选框；额度耗尽的 Trial Quota pill 需要灰化；前沿模型 `<select>` 之前是浏览器默认蓝白外观；互评分数需要按区间分档配色。调整内容：1. `.custom-select-wrapper.is-locked` 灰化免费模型自定义下拉框（modelSelect/imageModelSelect），customTrigger/imageCustomTrigger 的点击处理器检查这个 class 拦截展开，frontierOnlyToggle(Image) 与 Clear 按钮联动切换/清除。2. `.trial-quota-badge.is-exhausted` 灰化样式，Jinja 渲染时按 `limit - used <= 0` 初始判定，`updateOneQuotaBadge()` 之后每次刷新按同一条件切换。3. Claude/ChatGPT/Gemini 的 5 个 model-select-group `<select>` 加 `appearance:none` + 自定义箭头 + 深色聚焦描边，仿照 `.custom-select-trigger` 视觉。4. `.review-score-badge` 拆成 `score-low`(0-33,#E8F5E9)/`score-mid`(34-66,#66BB6A)/`score-high`(67-100,#1B5E20) 三档,index.html 和 history.html 的 `renderPeerReviews()` 都改用新增的 `scoreColorClass()` 判定。测试见 tests/test_frontier_ui_polish.py。`

`[互评稳定性修复] 更新原因：provider>=6 时一轮互评会让同一个免费 provider（PollinationsAI/Groq/OpenRouterFree 等）同时收到多个并发请求，触发 429 风暴；重试次数不够、退避太短，最终掉进 80 分兜底，兜底文案里还残留了另一段没解析成功的 JSON 导致分数对不上。调整内容：1. `run_peer_review()` 重试次数从 1 次加到 2 次（共 3 次尝试，`PEER_REVIEW_MAX_ATTEMPTS`），退避随尝试次数递增（`_peer_review_retry_wait()`）而不是固定 2~3s。2. `run_cross_peer_review()` 新增按 reviewer 身份（kind+provider）加的独占锁，串行化"打向同一个 reviewer"的并发任务，避免同一限流后端被同批任务集中打；外层 `future.result()` 超时改由公式 `max_reviewer_queue_depth * _peer_review_single_worst_case_seconds() + PEER_REVIEW_FUTURE_TIMEOUT_BUFFER` 现算，不再是写死的 32。3. `parse_peer_review_json()` 改成按花括号深度配平扫描出所有候选 JSON 子串（`_extract_balanced_json_candidates()`），从最后一段开始尝试解析，修掉"模型自我纠正、文本里出现两段 JSON 时首尾掐头去尾拼出无效字符串"的解析 bug。测试见 tests/test_peer_review_reliability.py，`test_main_whitebox.py`/`test_main_graybox.py` 里依赖旧重试次数/旧超时常量的用例已同步更新。`

`[前沿 select 配色补丁 + 免费模型下拉锚定修复] 更新原因：上一条"前端视觉打磨"记录里的 `appearance:none` 只重绘了 `<select>` 收起状态的框体，展开后的原生 `<option>` 弹层仍是系统蓝白默认色；另外"Select free models"下拉框用 `position:absolute`，未勾选任何 free provider（联合全部 provider 模型、列表最长）时展开会把 document 的 scrollHeight 撑高出 `.app-layout` 自身盒子高度，滚动到底部会在锚定的 `.left-sidebar` 下方露出空白。调整内容：1. 给 5 个 model-select-group 的 `<select> option`/`option:checked` 补黑白配色（未选中白底黑字、已选中黑底白字），原生弹层的 hover 态是浏览器平台限制,CSS 管不到。2. `.custom-options`（`#customOptions`/`#imageCustomOptions`）改成 `position:fixed`,不再计入非 fixed 祖先的布局尺寸；新增共享的 `positionCustomOptions()` 用 `getBoundingClientRect()` 在展开前现算 top/left/width（除以 `--page-zoom` 换算回逻辑像素）,并在真实页面滚动时收起两个下拉框防止悬空错位,展开态额外加 `overscroll-behavior: contain` 防止内部滚动链式带动外层页面。测试见 tests/test_free_model_dropdown_fixed_and_native_select_options.py。`

`[免费模型下拉改为逐 provider 绑定] 更新原因：共享的"Select free models"下拉框此前把所有勾选 provider 的模型合并成一个并集,选中的模型全局应用到每个勾选 provider,用户没法respectively给不同 provider 分别指定模型,也看不出每个 provider 实际用了哪个模型。调整内容：1. 对话表单和图片表单都引入 providerCheckOrder/imageProviderCheckOrder（勾选顺序队列）+ providerModelSelections/imageProviderModelSelections（每个 provider 各自记住的模型）,共享下拉框现在始终绑定"最新一次被勾选"的 provider（activeModelProvider()/activeImageModelProvider()）,只显示这一个 provider 自己的模型列表；0 个 provider 勾选时下拉框只剩 default 选项且灰化锁定（复用 is-locked,由新增的 updateModelSelectLockState()/updateImageModelSelectLockState() 统一算出锁定态 = frontier-only 或没有绑定目标）。2. 每个 provider 卡片下方的小字（refreshProviderModelLabel()/refreshImageProviderModelLabel()）跟着这个 provider 自己的选择实时更新："Default: X"（未覆盖或选择等于默认值）/"Selected: Y"（选择不同于默认值）。3. 提交时不再发送单个全局 model 字段,改成 provider_models 字典（{provider: model}）；main.py 的 compare_providers()/generate_images() 新增同名的逐 provider 覆盖参数,缺失的 provider 落回旧的全局 model 字段（向后兼容）,再落回各自默认模型。Clear Results 会连带清空这两套逐 provider 状态。测试见 tests/test_per_provider_model_selection.py（后端）与 tests/test_provider_model_dropdown_per_provider.py（前端逻辑）,tests/test_frontier_ui_polish.py 里两个断言旧锁定实现细节的用例已同步改成断言新的集中式锁定函数。`

`[前沿 model select 改建为自定义下拉组件] 更新原因：上一条"配色补丁"给原生 <select>/<option> 打的 CSS 补丁始终做不到"已选中黑色/未选中白色/hover 变绿/点击立即生效"——原生下拉弹层由操作系统合成渲染，:hover 和选中态背景色无法被 CSS 完全覆盖,展开后依旧是系统蓝白高亮。调整内容：Claude/ChatGPT/Gemini(文本) 和 Gemini/ChatGPT(图片) 共 5 个 model select 全部改建成与"Select free models"同构的 .custom-select-wrapper/.custom-select-trigger/.custom-options/.custom-option 组件（新增通用的 setupFrontierModelDropdown()）,原生 <select> 保留但 display:none,只作为 .value/.disabled 的状态存储,既有的启停用逻辑（syncClaudeModelSelectEnabled()、createFrontierProviderController() 等）零改动,靠 MutationObserver 监听 disabled 属性同步可视层的 is-locked 灰化。移除了已废弃的原生 select 配色 CSS。测试见 tests/test_frontier_model_custom_dropdown.py；tests/test_free_model_dropdown_fixed_and_native_select_options.py 与 tests/test_frontier_ui_polish.py 里断言旧原生 select 配色规则的用例已删除，tests/test_main_blackbox.py 两处 `<label for=...>` 断言同步改成无 for 属性的 `<label>`。`

`[前沿 model select 黑色高亮延迟] 更新原因：上一条改建成自定义下拉组件后，点击选项会让黑色高亮立即跳变到新选项，与面板收起动画同时发生，视觉紊乱；镜像"Select free models"下拉框已有的 MODEL_HIGHLIGHT_DELAY_MS 处理。调整内容：setupFrontierModelDropdown() 拆成 syncTriggerLabel()（文案/select.value/change 事件立即更新）和 syncSelectedHighlight()（.selected 类切换），点击选项时后者延后 MODEL_HIGHLIGHT_DELAY_MS（与面板收起动画同步）再执行，重新展开时立即 clearTimeout 并同步一次；MODEL_HIGHLIGHT_DELAY_MS 常量上移到 setupFrontierModelDropdown() 之前，与免费模型下拉框共用同一个值。测试见 tests/test_frontier_model_custom_dropdown.py 新增的 TestFrontierModelDropdownHighlightDelay。`

`[失败的 blind review 直接隐藏] 更新原因：互评失败后展示兜底文案（"80 pts: The system is busy..."）没有意义，还要硬等完整重试链拖慢体验；优先保证 provider response 速度，互评次之。调整内容：1. run_peer_review()/run_frontier_peer_review() 失败时返回 None 而不是兜底 review_result，run_cross_peer_review() 原有的 None 过滤逻辑据此把这条互评整条隐藏，不影响该 provider 自身 response 的展示。2. PEER_REVIEW_MAX_ATTEMPTS 从 3 降到 2（1 次重试），降低单条互评最坏耗时，future_timeout 公式自动同步。测试见 tests/test_peer_review_hidden_on_failure.py。`
