...
Cross-checked every claim in `resume_bullets.md` against `main.py`, `auth/`, and `tests/` directly (not just against this file's own section 12, since section 12 is itself one of the things being checked). One table per resume bullet, one row per individual factual/numeric claim.
...
|---|---|---|
|"3-stage concurrent inference pipeline using ThreadPoolExecutor"|3 stages genuinely exist (g4f compare, frontier chat, peer review), but only stage 1 and stage 3 use a server-side `ThreadPoolExecutor`. Stage 1: `main.py:2034-2036`. Stage 3: `run_cross_peer_review()`, `main.py:1032`. Stage 2 (`claude_chat`/`chatgpt_chat`/`gemini_text_chat`, `main.py:2381-2714`) contains no `ThreadPoolExecutor` at all (confirmed via `grep -n ThreadPoolExecutor main.py`, only 3 call sites in the whole file: `main.py:1032`, `2035`, `2843`); its concurrency comes from the browser firing 3 parallel `fetch()` calls, not a shared Python thread pool.|🟡 PARTIAL / IMPRECISE|
|"fan out prompts across 7 LLM APIs"|Correct count: `G4F_PROVIDERS` has 4 entries (`main.py:100-105`: Yqcloud, OperaAria, PollinationsAI, CohereForAI_C4AI_Command) + 3 frontier vendor SDKs (Claude `call_claude_model`, `main.py:1074`; OpenAI `call_chatgpt_model`, `main.py:1324`; Google `call_gemini_text_model`, `main.py:1266`) = 7 distinct backends.|✅ VERIFIED|
|"compute an N x (N-1) peer-review grid"|`run_cross_peer_review()` builds one task per ordered `(target, reviewer)` pair excluding self-review (`main.py:976-985`), dispatched to either `run_peer_review()` (g4f) or `run_frontier_peer_review()` (frontier) via `_dispatch()` (`main.py:1006-1016`) — this is the one stage that actually spans all 7 APIs through a single `ThreadPoolExecutor` (`main.py:1032`).|✅ VERIFIED|
|"cut comparison latency by 41.8% (17.2s to 10.0s)"|Numbers match CLAUDE.md section 12 verbatim (17.23s → 10.02s, 41.8%). But per section 12's own text this was measured only against "the live 4 provider `G4F_PROVIDERS` list," not the full 7-API surface the bullet's first clause describes. The underlying benchmark script is not committed anywhere in the repo (`tests/` has no such file — confirmed by directory listing); the only record of it is CLAUDE.md section 12 itself, not independently reproducible from the repo.|🟡 PARTIAL / IMPRECISE|
|"review-grid latency by 32.7% (70.8s to 47.7s)"|Same source, same caveat: section 12 states this used "the same 4 g4f providers," a 12-task grid (N=4), not a real 7-provider/42-task grid including live Claude/ChatGPT/Gemini calls. Script not committed to the repo.|🟡 PARTIAL / IMPRECISE|
|"100% call success rate"|Section 12: comparison stage "40/40 real calls succeeded," peer review "72/72 reviews landed (no dropped `None` reviews)" — both 100%, but two separate ad hoc measurements folded into one resume phrase, and both scoped only to the 4 g4f providers, never the 3 frontier SDKs.|🟡 PARTIAL / IMPRECISE|
...
|Resume claim|Codebase reality|Status|
|---|---|---|
|"stateless cancellation system using in-memory registries"|The registries are literally in-process, mutable Python dicts (`_PENDING_FRONTIER_REFUNDS`, `main.py:1467`; `_CANCELLED_HISTORY_REQUESTS`, `main.py:1518`) — inherently stateful, lost whenever a GAE instance recycles (CLAUDE.md section 3.2). "Stateless" describes the GAE deployment platform's constraint that shaped this design (no shared external store), not the cancellation mechanism itself, which necessarily holds state to do its job.|🟡 PARTIAL / IMPRECISE|
|"reconcile client aborts with server-side SDK calls"|Matches the code exactly: the comment block at `main.py:1446-1465` states `AbortController.abort()` only severs the client's fetch and cannot interrupt a blocking `anthropic.messages.create()`/`genai` call already running server-side; the ledger exists specifically to reconcile that gap.|✅ VERIFIED|
|"exact-once refund semantics across 200 high-concurrency trials"|CLAUDE.md section 12 records 200 trials × 25 racing threads (5,000 attempts), 0 duplicates/0 lost, plus a second 200/200 rejection pass — a real recorded result, but the script isn't committed anywhere (`tests/test_stop_generating_refund.py` has only ordinary sequential unit tests, no race/concurrency test — confirmed by reading the file's test names). Separately, `_consume_pending_frontier_refund()` (`main.py:1488-1495`) has no explicit lock: it does `entry = dict.get(...)` then `dict.pop(...)` as two separate operations, not one atomic compound op. "Exact-once semantics" implies a guarantee; what exists is an empirically-clean result resting on CPython's GIL and the operation's short runtime, not a mutex or atomic primitive.|🟡 PARTIAL / IMPRECISE|
...
|Resume claim|Codebase reality|Status|
|---|---|---|
|"custom exception classifiers to reverse-engineer undocumented vendor failure modes"|`_classify_google_genai_error()` (`main.py:328`) and `_classify_openai_error()` (`main.py:386`) both use `getattr()` duck-typing specifically because the real SDK exception classes live in private, underscore-prefixed submodules with no stable public import path (matches CLAUDE.md section 4.5). Claude's handling (inline in `call_claude_model()`, `main.py:1074` area) was confirmed against a real account to return 400 + `invalid_request_error` + "credit balance" in the message — not the 429, or the 403 + `billing_error`, shape the general docs imply (`tests/test_claude_integration.py:131-139`, `:154-165`).|✅ VERIFIED|
|"100% classification accuracy against live-verified error fixtures"|Only 2 shapes are actually live-verified against real vendor accounts: Claude's credit-balance-exhausted shape (`tests/test_claude_integration.py:131-139`) and Gemini's 429/`RateLimitError` quota-exhausted shape (`tests/test_gemini_integration.py:202-222`, "seen by calling all three Nano Banana models directly with a real but zero-quota GEMINI_API_KEY"). Two branches are explicitly documented as NOT live-verified: Gemini's 403/permission-denied path is "verified only against the [docs]" (`tests/test_gemini_integration.py:25`, matches this file's own section 9 admission that "the 403 path is docs only"), and the entire OpenAI/ChatGPT classifier (`main.py:386`, tested at `tests/test_new_frontier_providers.py:89-113`) has no real-account verification anywhere in the repo — no test or comment claims it was ever fired against a live OpenAI account. The "100% classified correctly" figure itself is real for the fixture set that was run (CLAUDE.md section 12: "All 8 classified correctly"), but "live-verified" overstates that set: most of it is docs-derived, not vendor-confirmed. Also, section 12's own wording ("8 synthetic exception fixtures (4 Google, 4 OpenAI) ... fed into the real `_classify_google_genai_error()` and `_classify_openai_error()`") never actually includes Claude's classifier — Claude's error handling is a separate inline branch in `call_claude_model()`, not one of the two functions that 8-fixture benchmark exercised — despite that same sentence citing the "live verified Anthropic ... shape" as part of what the fixtures were shaped to match. That's an internal inconsistency in section 12 itself.|🟡 PARTIAL / IMPRECISE|
