...
class TestGeneratedMediaCleanupFeatureRemoved(unittest.TestCase):
    """2026-07-05: the previous age-based sweep (cleanup_old_generated_media()/
    GENERATED_MEDIA_MAX_AGE_SECONDS, which used to run at the top of generate_images())
    was deliberately removed so that historical generated images referenced by the image
    Recents/history detail pages remain viewable indefinitely, instead of 404ing once
    they cross the old 1-hour age threshold. This is a regression guard, not a feature
    test -- it locks down that the symbols are gone, so a future edit doesn't
    reintroduce automatic deletion by copy-pasting an old snippet back in. See
    tests/test_main_blackbox.py::TestGenerateImagesNeverDeletesMedia for the behavioral
    (black-box) side of this same guarantee."""

    def test_cleanup_function_no_longer_exists(self):
        import main
        self.assertFalse(hasattr(main, 'cleanup_old_generated_media'))

    def test_max_age_constant_no_longer_exists(self):
        import main
        self.assertFalse(hasattr(main, 'GENERATED_MEDIA_MAX_AGE_SECONDS'))
